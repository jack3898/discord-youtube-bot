...
const redis: any = redisModule.createClient(config.redis_port);
...
export default class Queue {
	 ...
	 */
	add = async (string: string) => {
		const url: any = ytdl.validateURL(string) ? string : await findYtUrl(string, 1);
	 ...
	 */
	get = async (page, pageSize: any = config.paginate_max_results) => {
		...
		if (Number.isNaN(validatedPage) || Number.isNaN(validatedPageSize)) {
			throw TypeError(`The page and or page size provided are not a valid number.`);
		}
	...
	move = async (initialIndex, newIndex) => {
		...
		if (Number.isNaN(initialIndexAdjusted) || Number.isNaN(newIndexAdjusted)) throw Error('Invalid index values. Integers only.');

...
export default {
	...
	action: async (bot, msg, command) => {
		try {
			...
			const resolvedVideos: any = await Promise.all(urls.map(getVideoDetails));

			// This is for the MessageEmbed
			const fields = resolvedVideos.map((video: any, index) => {
				return {
			...
			// Ask the question!
			const question = await msg.reply(
				new MessageEmbed()
			...
			function listener(reaction) {
				...
				reacts.forEach((rawReaction, index) => {
					...
					if (resolvedReaction !== rawReaction) return;

...
export function onMessage(bot, msg) {
    ...
    if (bot.commands.has(command.cmd))
        bot.commands.get(command.cmd).action(bot, msg, command);
}
/**
...
export function onRedisReady(bot, redis) {
    ...
    if (config.redis_flush_on_start) {
        if (redis.flushall())
            console.log('Redis database flushed - you can turn this off in config.json.');
    }
}
